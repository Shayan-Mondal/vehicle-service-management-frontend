...
export function MechanicUpsert() {
  ...
  const [mechanicsAge, setMechanicsAge] = useState(
    state.mechanic.refmec.mechanicsAge
  );
  const [mechanicsMobile, setMechanicsMobile] = useState(
    state.mechanic.refmec.mechanicsMobile
  );
    ...
    <div className="row">
      ...
      <div className="col-12 col-md-6">
        <h3 className="alert alert-warning">
          {state.mechanic.refmec.mechanicsId
            ? "Update Mechanic"
            : "Create Mechanic"}
        </h3>
        ...
        {errorOperation && <div className="alert alert-danger">{message}</div>}
